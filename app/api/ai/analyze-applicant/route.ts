...
export async function POST(request: Request) {
  try {
    ...
    console.error('[analyze-applicant] unhandled error:', err)
...
async function handlePost(request: Request) {
  ...
  const [applicant] = await db.select().from(applicants)
    .where(and(eq(applicants.id, applicantId), eq(applicants.orgId, orgId)))
    .limit(1)
...
Analyse this applicant and respond ONLY with valid JSON - no prose, no code fences, nothing else:
...
Sort course_matches by score descending. Use exact course names from the list above.`
  ...
  if (block.type !== 'text') return NextResponse.json({ error: 'Unexpected AI response type' }, { status: 500 })
  ...
  try {
    ...
    console.error('AI JSON parse failed. Raw:', responseText)
  ...
  return NextResponse.json({
    ...
    courseMatches: parsed.course_matches,
    matchedCourseId,
  })
}
